...
export default function Card({ card, placed = false, amIP1 }: CardProps) {
  ...
  return (
    <div
      className={`flex flex-col justify-between ${placed ? 'border border-gray-300' : 'bg-white'} w-full h-full ${amIP1 ? placed ? (card?.placedByPlayerOne ? 'bg-green-400' : 'bg-red-400') : '' : placed ? (card?.placedByPlayerOne ? 'bg-red-400' : 'bg-green-400') : ''} rounded-lg`}
    >

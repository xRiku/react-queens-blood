...
export default function Board() {
  const rows = 3;
  const cols = 7;
  const tilesElements = new Array(rows).fill(0).map(() => new Array(cols).fill(0));
  const matrix = new Array(rows).fill(0).map(() => new Array(cols - 2).fill(0));
  const [tiles, setTiles] = useState<Tile[][]>(matrix.map((value, index) => {
  ...
  const monsterCard: Card = {
    name: 'Monster',
    pawnsPositions: fillMonsterPositions([[1,0], [0,1]]),
    points: 2,
    pawnsCost: 1,
  }
  ...
  function canAddCardToPosition (card: Card, playerOnePoints: number[], playerTwoPoints: number[], position: Tile) {
  ...
  for(let i = 0; i < rows; i++) {
    tilesElements[i][0] = <div className={`bg-white h-60 w-full flex items-center justify-center border-solid border-2 border-black`} key={`${i}-${0}`}>
          <div className="h-28 w-28 border-solid border-4 text-3xl border-green-300 rounded-full flex justify-center items-center">{playerOnePoints[i]}</div>
        </div>;
    tilesElements[i][cols - 1] = <div className={`bg-white h-60 w-full flex items-center justify-center border-solid border-2 border-black`} key={`${i}-${cols - 1}`}>
  ...
  for(let i = 0; i < rows; i++) {
    for(let j = 1; j < cols - 1; j++) {
      ...
      tilesElements[i][j] = <div className={`${color} h-60 w-full border-solid border-2 flex justify-center items-center border-black`} key={`${i}-${j}`}>
